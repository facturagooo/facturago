...
const isSchemaError = (error: any): boolean => {
    ...
    return (
        ...
        msg.includes('cannot find table')
    );
};
    ...
    settings: {
        get: async (): Promise<CompanySettings | null> => {
            try {
                ...
                if (error) {
                    if (isSchemaError(error)) {
                        console.warn("Table 'settings' not yet created or loading in schema cache, using default settings:", error.message);
                        const fallbackSettings: CompanySettings = {
                            id: 'default-settings',
                            companyName: 'Facturago',
                            primaryColor: '#10b981',
                            headerTextColor: '#ffffff',
                            tableHeaderBgColor: '#10b981',
                            showTableBorders: true,
                            showAmountInWords: true,
                            documentInfoPosition: 'right',
                            showExpiryDate: true,
                            showUnitInPDF: true,
                            logoWidth: 200,
                            stampWidth: 220,
                            ...
                            defaultTva: 20,
                            invoiceNumbering: { prefix: 'FA', yearFormat: 'YYYY', startNumber: 1, padding: 5, separator: '/' },
                            quoteNumbering: { prefix: 'DE', yearFormat: 'YYYY', startNumber: 1, padding: 5, separator: '/' },
                            deliveryNoteNumbering: { prefix: 'BL', yearFormat: 'YYYY', startNumber: 1, padding: 5, separator: '/' },
                            purchaseOrderNumbering: { prefix: 'BC', yearFormat: 'YYYY', startNumber: 1, padding: 5, separator: '/' },
                            creditNoteNumbering: { prefix: 'AV', yearFormat: 'YYYY', startNumber: 1, padding: 5, separator: '/' },
                        };
                    ...
                    console.warn("Error fetching settings:", error?.message || error?.details || error);
                    return null;
                }
