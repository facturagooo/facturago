            ...
            <div className={`relative w-full h-full md:h-auto md:max-h-[95vh] md:max-w-6xl bg-white md:rounded-3xl shadow-2xl transition-all duration-300 ease-out flex flex-col ${isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0 translate-y-10 md:translate-y-0'}`}>
                ...
                <div className="px-3 md:px-6 py-5 overflow-y-auto custom-scrollbar flex-1 space-y-6 pb-24 md:pb-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {(!showReferenceField || !showPurchaseOrderField || !showExpiryDateField || !showSubjectField || !showPaymentMethodField) && (
                            <div className="flex flex-wrap items-center gap-2 pt-1 pb-1 md:col-span-2">
                                ...
                                {!showExpiryDateField && (
                                    <button 
                                        ...
                                        className="text-xs font-bold text-emerald-700 hover:text-emerald-800 flex items-center gap-1.5 bg-emerald-50 hover:bg-emerald-100/80 px-3.5 py-2.5 rounded-xl border border-emerald-200/80 transition-all hover:scale-[1.02] active:scale-[0.98] shadow-xs"
                                    >
                                        <Plus size={14} className="stroke-[2.5]" /> {t('addExpiryDate')}
                                    </button>
                                )}
                                {!showSubjectField && (
                                    <button 
