    ...
    useEffect(() => {
        const loadData = async (isRetry = false, silent = false) => {
            ...
            try {
                ...
                const clientsData = await dbService.clients.getAll().catch(e => { console.warn("Clients load issue:", e); return []; });
                const productsData = await dbService.products.getAll().catch(e => { console.warn("Produits load issue:", e); return []; });
                const suppliersData = await dbService.suppliers.getAll().catch(e => { console.warn("Fournisseurs load issue:", e); return []; });
                const settingsData = await dbService.settings.get().catch(e => { console.warn("Paramètres load issue:", e); return null; });
                ...
                const quotesData = await dbService.quotes.getAll().catch(e => { console.warn("Devis load issue:", e); return []; });
                const invoicesData = await dbService.invoices.getAll().catch(e => { console.warn("Factures load issue:", e); return []; });
                const deliveryData = await dbService.deliveryNotes.getAll().catch(e => { console.warn("Bons de livraison load issue:", e); return []; });
                const purchaseOrdersData = await dbService.purchaseOrders.getAll().catch(e => { console.warn("Commandes load issue:", e); return []; });
